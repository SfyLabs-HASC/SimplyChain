...
import { ConnectButton, useActiveAccount, useReadContract, useSendTransaction } from "https://esm.sh/thirdweb/react";
import { createThirdwebClient, getContract, prepareContractCall } from "https://esm.sh/thirdweb";
...
// Placeholder for ABI, as it was in a separate file.
const supplyChainABI = [
  ...
  },
  {
    ...
    "inputs": [
      ...
      { "name": "_imageIpfsHash", "type": "string", "internalType": "string" }
    ],
    "outputs": [],
  ...
  },
  {
    ...
    "name": "addStepToBatch",
    "inputs": [
      { "name": "_batchId", "type": "uint256", "internalType": "uint256" },
      { "name": "_eventName", "type": "string", "internalType": "string" },
      { "name": "_description", "type": "string", "internalType": "string" },
      { "name": "_date", "type": "string", "internalType": "string" },
      { "name": "_location", "type": "string", "internalType": "string" },
      { "name": "_attachmentsIpfsHash", "type": "string", "internalType": "string" }
    ],
    "outputs": [],
    "stateMutability": "nonpayable"
  },
...
const abi = supplyChainABI;

// Placeholder for RegistrationForm component
const RegistrationForm: React.FC<{ walletAddress: string }> = ({ walletAddress }) => {
  return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] text-center p-6">
      <h2 className="text-2xl font-bold mb-4">Registra la tua Azienda</h2>
      <p className="mb-4">Il tuo wallet ({walletAddress}) non è ancora registrato. Compila il modulo per iniziare.</p>
      <div className="w-full max-w-md">
        <input type="text" placeholder="Nome Azienda" className="w-full p-3 rounded-lg bg-gray-800 border border-gray-700 text-foreground mb-4" />
        <button className="primary-gradient text-white px-4 py-2 rounded-2xl font-semibold hover:scale-105 transition w-full">
          Registra
        </button>
      </div>
    </div>
...
// Placeholder for TransactionStatusModal component
const TransactionStatusModal: React.FC<{
  ...
  return (
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
      <div className="bg-card p-6 rounded-2xl border border-border max-w-sm w-full text-foreground text-center">
        <div className="text-4xl mb-4">{statusInfo[status].icon}</div>
        <h3 className="text-xl font-bold mb-2" style={{ color: statusInfo[status].color }}>
          {status === 'loading' ? 'In corso...' : status === 'success' ? 'Successo!' : 'Errore'}
        </h3>
        <p className="text-muted-foreground">{message}</p>
        {status !== 'loading' && (
          <button onClick={onClose} className="primary-gradient text-white px-4 py-2 rounded-2xl font-semibold hover:scale-105 transition mt-6 w-full secondary">
            Chiudi
          </button>
        )}
      </div>
...
        transform: translateY(-2px);
        border-color: #8b5cf6; /* MODIFICA: Bordo viola */
        box-shadow: 0 8px 25px rgba(0, 0, 0, 0.4), 0 0 8px rgba(139, 92, 246, 0.5); /* MODIFICA: Ombra originale + bagliore viola */
      }

      .glass-card rounded-2xl p-6 tech-shadow hover:shadow-lg transition h3 { 
        font-size: 1.6rem; /* MODIFICA: Titolo più grande */ 
        font-weight: 700; /* MODIFICA: Titolo in grassetto */
        color: #ffffff; 
        margin: 0 0 1rem 0;
...
        .glass-card rounded-2xl p-6 tech-shadow hover:shadow-lg transition h3 { 
          font-size: 1.75rem; /* MODIFICA: Titolo più grande su tablet */
        }
